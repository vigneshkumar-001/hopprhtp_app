...
class PaymentLinkReadyScreen extends StatefulWidget {
  ...
  /// The just-created transaction, so "View Transaction" can open its detail
  /// screen directly instead of the buyer-facing checkout preview.
...
class _PaymentLinkReadyScreenState extends State<PaymentLinkReadyScreen> {
  ...
  /// Opens the hosted web checkout in an in-app browser — debug/admin only,
  /// so a seller can sanity-check what the buyer will see. Never the
  /// production seller CTA: the seller must not pay their own transaction.
  /// When the buyer pays, the page hands control back and we surface the
  /// Link-transaction screen.
  Future<void> _openCheckout() async {
    final result = await AppNav.push<CheckoutResult>(
      context,
      CheckoutWebViewScreen(code: _code, lime: AppAccent.of(context).isLime),
    );
    if (result?.outcome == PaymentOutcome.success && mounted) {
      AppNav.push(context, const LinkTransactionScreen());
    }
  }

  /// Primary seller CTA — opens this transaction's own detail screen. Falls
  /// back to popping back to the dashboard if the transaction wasn't handed
  /// through (shouldn't happen on the real create-transaction path).
  ///
  /// Uses [AppNav.pushAndClearToFirst] so the completed create flow (and this
  /// success screen) are dropped from the stack: Back from the detail returns
  /// to Home, never to Payment Setup / Create Transaction.
  void _viewTransaction() {
    final tx = widget.tx;
    if (tx == null) {
    ...
    return AppScaffold(
      ...
      body: Column(
        ...
        children: [
          ...
          const SizedBox(height: AppSizes.sm),
          Text(
            'Share this link with the buyer to complete escrow payment. '
            'Only the buyer should pay — you can track its status from '
            'View Transaction.',
            textAlign: TextAlign.center,
            style: AppText.body,
          ),
          ...
          // Debug-only: preview exactly what the buyer will see. Never shown
          // in production — sellers must not be steered toward paying their
          // own transaction.
          if (kDebugMode) ...[
            const SizedBox(height: AppSizes.md),
            Center(
              child: TextButton.icon(
                ...
                label: Text(
                  "Debug: Preview Buyer's Payment Page",
                  style: AppText.caption,
                ),
              ),
            ),
